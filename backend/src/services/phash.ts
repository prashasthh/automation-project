...
/**
 * Compute a dHash (difference hash) from an already-fetched image buffer.
 * Returns a 64-bit BigInt representing the hash.
 */
export async function dHashFromBuffer(buffer: Buffer): Promise<bigint> {
...
/**
 * Compute a dHash for an image URL (fetches, then hashes).
 * Returns 0n on any failure so callers won't crash.
 */
